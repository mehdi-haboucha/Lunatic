...
test('Questionnaire Controles Numériques', async ({ page }) => {
	await page.goto(
		'https://visu-stromae-v2-sandbox.demo.insee.io/visualize?questionnaire=https%3A%2F%2Fpogues-back-office-sandbox.demo.insee.io%2Fapi%2Fpersistence%2Fquestionnaire%2Fjson-lunatic%2Fkzfezgxb'
	);
	...
	await page
		...
		.fill('200');
	await expect(page.getByText('QREEL vaut : 200')).toBeVisible();
	await page.getByRole('button', { name: 'Continuer' }).click();
	await expect(
		page
			.getByRole('dialog', { name: 'Des points requièrent votre attention.' })
	...
	// test avec valeur superieur à 9
	await page.getByLabel('➡ 8. Test d’égalité - Message si = 4').fill('11');
	await page.getByRole('button', { name: 'Continuer' }).click();
	await expect(
		page
			.getByRole('dialog', { name: 'Des points requièrent votre attention.' })
	...
	// test avec valeur superieur à 9
	await page.getByLabel('➡ 9. Test d’égalité à 0 - Message si = 0').fill('11');
	await page.getByRole('button', { name: 'Continuer' }).click();
	await expect(
		page
			.getByRole('dialog', { name: 'Des points requièrent votre attention.' })
